...
import { setCurrentTrack } from "@/lib/slices/player-slice";
...
export default function FavoriteSongs() {
  ...
  }, [accessToken, fetchFavoriteSongs]);
      ...
      <div className="w-full h-[70vh] overflow-y-scroll scrollbar-hidden-favorite p-4">
        {favoriteSongs.length > 0 && (
          <div className="w-full flex items-center gap-9">
            ...
            <div>
              <h1 className="text-2xl font-bold">Favorite tracks</h1>
            </div>
          </div>
        )}
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="text-gray-500 text-left">
                <th className="pb-2 border-collapse border-b-[1px]">#</th>
            ...
            <tbody>
              {favoriteSongs.map((song, i: number) => (
                <tr
                  ...
                  onClick={() => dispatch(setCurrentTrack(song.track.id))}
                >
                  ...
                  <td className="flex items-center gap-2  py-2 px-3 text-white">
                    <Image
                      ...
                      src={
                        favoriteSongs[i].track.album.images[0]?.url ||
                        "/placeholder-image.jpg"
                      }
                      alt={
                        favoriteSongs[i].track.album.name ||
                        "No Image Available"
                      }
                    />
